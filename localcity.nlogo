...
to go
  ...
  ask turtles with [ awareness > 15 ] [ set awareness 15 ]
  ask turtles with [ awareness < 0 ] [ set awareness 0 ]
...
to calculate-diffusion-index
  let value (random 100) - (random 50)
  output-print (word "Diffusion index: " value)
end



to reset-center-memory
  ask patches with [pcolor = green or pcolor = blue] [
    set non-usage 0
  ]
end

to dummy-check-environment
  if count turtles > 50 [ output-print "Environment is crowded." ]
end
@#$#@#$#@
GRAPHICS-WINDOW
315
